          ...
          <section className="bg-white rounded-lg shadow-lg border border-gray-200 flex flex-col" aria-labelledby="input-heading">
            ...
            <div className="flex-1 p-4">
              <label htmlFor="input-textarea" className="sr-only">
                {mode === 'xmlToJson' ? 'XML input field' : 'JSON input field'}
              </label>
              <textarea
                id="input-textarea"
                value={input}
                onChange={(e) => handleInputChange(e.target.value)}
                aria-label={mode === 'xmlToJson' ? 'XML input for conversion' : 'JSON input for conversion'}
                placeholder={
                  mode === 'xmlToJson' 
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 border-b rounded-t-lg">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  aria-label={output ? 'Download converted file' : 'Download not available'}
                ...
                <button
                  ...
                  disabled={!output}
                  aria-disabled={!output}
                  aria-label={copied ? 'Output copied to clipboard' : output ? 'Copy output to clipboard' : 'Copy not available'}
                  className={`flex items-center space-x-2 px-3 py-2 rounded-lg transition-all duration-200 ${
                    output
              ...
              {error ? (
                ...
                <>
                  <label htmlFor="output-textarea" className="sr-only">
                    {mode === 'xmlToJson' ? 'JSON output field' : 'XML output field'}
                  </label>
                  <textarea
                    id="output-textarea"
                    value={output}
                    readOnly
                    aria-label={mode === 'xmlToJson' ? 'Converted JSON output' : 'Converted XML output'}
